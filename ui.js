...
+'<div class="ds">⏱ '+(r.tm||'-')+' · 철 '+rnd(n.fe)+'mg · 단백 '+rnd(n.p)+'g'+(x||'')+'</div>'+comboTag(r)+'</div>'
...
function todaySum(){var f={p:0,fe:0,ca:0,zn:0},m={p:0,fe:0,ca:0,zn:0},ml=0,cnt=0;
...
return '<div class="nrow"><div class="nhd"><div class="nnm"><i class="ndot" style="background:'+x.col+'"></i>'+x.nm+' <span class="badge '+lv+'">'+lvIco(x.pc)+'</span></div><div><div class="npc" style="color:'+lvCol(x.pc)+';font-size:16px">'+Math.round(x.pc)+'%</div><div class="nval">'+rnd2(x.v)+' / '+rnd2(x.goal)+x.u+'</div></div></div>'
+'<div class="bar" style="height:17px"><i class="solid" style="width:'+Math.min(100,x.pc)+'%;background:'+x.col+'"></i><span class="goal" style="left:calc(100% - 3px)"></span></div></div>'}
...
if(n.t.vc<10&&n.t.fe>0.3)msg.push('비타민C 재료가 적습니다. <b>브로콜리·파프리카·토마토·양배추</b>를 곁들이면 비헴철 흡수가 2~3배 올라갑니다.');
if(n.meat>15&&n.t.vc>=20)msg.push('고기(헴철) + 비타민C 조합으로 <b>철분 흡수 조건이 좋습니다</b> 👍');
return msg.length?'<div class="fe">🩸 <b>철분 흡수 코칭</b><br>'+msg.join('<br>')+'<div class="mu" style="font-size:10px;margin-top:5px">'+sT('fe')+'</div></div>':''}
...
+(k==='fe'?'<div class="cd" style="background:#FDF3F0;border-left:3px solid #E85536;font-size:12px"><b>🩸 철분은 함께 먹는 재료가 중요해요</b><p class="mu" style="margin:6px 0 0">고기·생선의 철분은 흡수가 잘 되지만, 식물성 철분은 그대로면 흡수율이 낮습니다. <b>파프리카·브로콜리·토마토</b> 같은 비타민C 재료를 곁들이면 흡수가 크게 올라가고, 반대로 <b>우유·치즈</b>를 같이 주면 흡수가 방해됩니다.</p><div style="margin-top:6px">'+sT('fe')+'</div></div>':'')
